...
def thread_proc(q,thread_number,transcript_list):
    ...
    for transcript in transcript_list:
        ...
        position_lists[chrom+"_"+str(pos)] = list()
        ...
        for i in range(pos-args.start,pos+args.start):
            ...
            for line in content:
                ...
                if pos_found == i:
                    ...
                    position_lists[chrom+"_"+str(pos)].append(coverage)
                    continue
            if not found:
                position_lists[chrom+"_"+str(pos)].append(0.)
        TMP_COVERAGE_BED_OUTPUT.close()
...
parser.add_argument('-pos','--position-file', dest='pos_file',
                   help='Position file',required=True)
...
POSITION = open(args.pos_file,"r")
target_content = list()
for target in POSITION.readlines():
   if int(target.split("\t")[1]) > 10000:
       target_content.append(target)
...
for thread in range(0,args.threads):
    for key in thread_position_list[thread].keys():
        position_lists_all[key] = thread_position_list[thread][key]
    

#output data
sys.stderr.write("--------------------------------------------------\n")
sys.stderr.write("\n"+str(len(position_lists_all))+" TSS analyzed\n")
sys.stderr.flush()

for tss in position_lists_all.keys():
    for number in position_lists_all[tss]:
