...
for line in BROADNORM.readlines():
    if line == "\n" or "Gene" in line:
        continue
    info = line.split("\t")
    broad_cov_norm_dict[info[0]] = info[2].rstrip()
BROADNORM.close()
...
OUTPUT.write("Gene\tType\t5'Slope\t3'Slope\tBroad TSS Coverage\tBroad TSS Coverage Norm\tSmall TSS coverage\n")
for gene in broad_cov_dict.keys():
    OUTPUT.write(gene+"\t"+expressed_dict[gene]+"\t"+slope_dict[gene]+"\t"+ broad_cov_dict[gene]+"\t"+broad_cov_norm_dict[gene]+"\t"+small_cov_dict[gene]+"\n")
OUTPUT.close()
